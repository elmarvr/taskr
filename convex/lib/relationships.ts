 ...
 */
export async function getOneFrom<TableName extends TablesWithLookups, Field extends LookupFieldPaths<TableName>>(
  db: DatabaseReader,
  table: TableName,
  field: Field,
 ...
 */
export async function getManyFrom<TableName extends TablesWithLookups, Field extends LookupFieldPaths<TableName>>(
  db: DatabaseReader,
  table: TableName,
  field: Field,
  value: FieldTypeFromFieldPath<Doc<TableName>, Field>
...
type LookupAndIdFilePaths<TableName extends TablesWithLookups> = {
  [FieldPath in IdFilePaths<TableName, TableNames>]: LookupFieldPaths<TableName> extends FieldPath ? never : true;
...
type JoinTables = {
  [TableName in TablesWithLookups]: LookupAndIdFilePaths<TableName> extends never ? never : TableName;
}[TablesWithLookups];
...
export async function getManyVia<
  ...
  TargetTableName extends TableNames = FieldTypeFromFieldPath<Doc<JoinTableName>, ToField> extends Id<
    infer TargetTableName
  >
    ? TargetTableName
    : never
>(
  ...
  return asyncMap(await getManyFrom(db, table, fromField, value), (link) => db.get((link as any)[toField]));
}
